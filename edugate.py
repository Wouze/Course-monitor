...
class EdugateClient:
    ...
    def _catalog_html_reused(self):
        if self._courses_url:
            try:
                resp = self._session.get(self._courses_url, timeout=REQUEST_TIMEOUT)
    ...
    def _login_and_open_catalog(self):
        ...
        self._courses_url = None
        try:
            login_page = self._session.get(LOGIN_URL, timeout=REQUEST_TIMEOUT)
            ...
            reg_page = self._session.get(REGISTRATION_URL, timeout=REQUEST_TIMEOUT)
    ...
    def _follow_add_courses(self):
        add_response = self._session.get(
            f"{ADD_COURSES_URL}?reg={random.random()}", timeout=REQUEST_TIMEOUT
        )
        match = re.search(r'window\.location\.replace\("([^"]+)"\)', add_response.text)
        if not match:
            return None, None
        url = BASE_URL + match.group(1)
        courses = self._session.get(url, timeout=REQUEST_TIMEOUT)
